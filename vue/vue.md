...
编辑prod.env.js
	...
	module.exports = {
  		NODE_ENV: '"production"',
  		API_ROOT: '"http://119.23.74.3:19100"'
	}
	
**在代码中调用设置好的参数**	
在封装axios请求的方法中配置api接口地址  

	let root=process.env.API_ROOT	
	
###二十五、webpakc在打包第三方库moment.js时会将moment的所有locale都打包进去，到这vendor.js过大

**解决办法：**
在build/webpack.prod.conf.js中添加如下代码:(两者择一即可)  
